...
@Transactional
public class BettingSimulatorImlIntegrationTest {
	...
	private static String bettingConfigFile = "src/test/resources/simulator_test1";
	...
	public void testSimulate() {
		...
		assertEquals(2,report.getNumOfMatchedBets());
		assertEquals(0.5052,report.getTotalLiability(),0.0001);
	}
		...
		List<HistRunnerPrice> histRunnerPrices = new ArrayList<HistRunnerPrice>();
			...
			for (int recordTime = 0; recordTime < 20; recordTime++) {
				...
				for (MarketDetailsRunner runnerDetails: market.getRunners()) {
					...
					histRunnerPrice.setPriceToBack(4 - (recordTime*0.1) - 0.02);
					histRunnerPrice.setPriceToLay(4 - (recordTime*0.1) + 0.02);
